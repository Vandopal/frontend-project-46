...
const mapping = {
  ...
  added: (node, iter, path) => `'${[...path, node.key].join('.')}' was added with value: ${typeof node.value === 'object' ? '[complex value]' : node.value}`,
  deleted: (node, iter, path) => `'${[...path, node.key].join('.')}' was removed`,
  unchanged: () => ``,
  changed: (node, iter, path) => {
    const { key, value1, value2 } = node;
    const data1 = `'${[...path, key].join('.')}' was updated. From ${typeof value1 === 'object' ? '[complex value]' : value1} to ${value2}`;
    return [data1];
  },
};
